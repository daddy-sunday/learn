...
import com.zhiyuan.zm.raft.constant.TaskType;
...
public class AppTest {
  ...
  public void testSleep(){
    ...
    thread.interrupt();
    System.out.println(thread.isAlive()+" "+thread.getState()+" "+Thread.interrupted());
  //  CompletableFuture.runAsync(() -> interruptSleep(thread));
    try {
      Thread.sleep(10000);
    } catch (InterruptedException e) {
      System.out.println("我被人打扰了");
    }
    System.out.println(thread.isAlive()+" "+thread.getState()+" "+Thread.interrupted());
  ...
  private void interruptSleep(Thread thread){
    try {
      Thread.sleep(1000);
    } catch (InterruptedException e) {
    ...
    System.out.println(thread.isAlive()+" "+thread.getState()+" "+thread.isInterrupted());
    thread.interrupt();
    System.out.println(thread.isAlive()+" "+thread.getState()+" "+thread.isInterrupted());
  }
